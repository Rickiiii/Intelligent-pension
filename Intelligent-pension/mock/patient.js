...
module.exports = {
  [`POST ${apiPrefix}/patient/search`](req, res) {
    ...
    if (Object.keys(req.body).length === 0 || (userName === '' && !id == true)) {
      const _data = dataSource.slice()
      _data.reverse()
      res.json({ success: true, message: 'Ok', data: _data })
    } else if (!id == true && userName !== '') {
      const user = dataSource.filter(item => item.userName === userName)
      res.json({ success: true, message: 'Ok', data: user })
    } else if (userName === '' && id !== '') {
      const user = dataSource.filter(item => item.id == id)
      res.json({ success: true, message: 'Ok', data: user })
    } else {
      const user = dataSource.filter((item) => { return item.id == id && item.userName === userName })
      res.json({ success: true, message: 'Ok', data: user })
    }
  },  
  ...
  [`POST ${apiPrefix}/patient/pay`](req, res) {
    const {
      payMoney, payType, payRemark, id 
    } = req.body
    const payload = {
      payMoney,
      payType,
    ...
    if (!id === true) {
      res.json({ success: false, message: 'id为空', data: [] })
    } else {
      paiedList.push(payload)
      res.json({ success: true, message: 'Ok', data: [] })
